    ...
    let answer2=req.body.Answer2;
        ...
        x.then(function(res){
            if(res.length==0){
                ...
                let select1=`SELECT password FROM Users WHERE (username='${username}')`;
                DButilsAzure.execQuery(select1);
                resulte.json({password:res[0].password});
            }
        });
...
//route getUser
router.post('/accounts/GetUser',function(req,resulte){
...
router.get('/accounts/GetCounterPointsOfUser',function(req,resulte){
    let token=req.decoded.payload;
    let username =token.username;
...
//route UpdateComment
router.put('/accounts/AddComment',function(req,resulte){
    let token=req.decoded.payload;
    let username=token.username;
...
router.put('/accounts/AddRank',function(req,resulte){
    let token=req.decoded.payload;
    let username=token.username;
    let pointId=req.body.PointID;
